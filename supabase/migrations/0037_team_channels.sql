-- ─────────────────────────────────────────────────────────────────────────
-- WonderFlow OS — real, editable communication channels (per-tenant).
-- Replaces the fixed 4-name channel list with real create/rename channels +
-- an audience (all team members, or specific employees). Note: employees
-- don't have individual WonderFlow logins (only account members do), so
-- "specific employees" is organizational labeling, not enforced access
-- control, until employees can log in individually.
-- ─────────────────────────────────────────────────────────────────────────
...
-- Backfill: give every org that already has posts (old fixed-channel model) a
-- real 'general' channel, so existing data still lands somewhere sensible.
