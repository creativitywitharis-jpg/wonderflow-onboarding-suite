-- ─────────────────────────────────────────────────────────────────────────
-- WonderFlow OS — ownership transfer + whole-business deletion (solo owners
-- only). Member hard-delete needs no migration: existing memberships RLS
-- (0020_membership_roles + successor) already lets owner/admin delete a
-- non-owner row; that's wired up purely client-side.
-- ─────────────────────────────────────────────────────────────────────────

-- Atomic role swap — both updates succeed or neither does, so there's never
...
-- Solo-owner-only whole-business deletion. organizations cascades to every
-- table in the app (every org_id column across ~40 tables references it
-- on delete cascade), so this one delete removes everything.
